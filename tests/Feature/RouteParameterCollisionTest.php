...
use Goldnead\Activity\Tests\TestCase;
 ...
 *
 * Here the shape of "belongs to it" is the `activity` prefix plus a capital. This
 ...
 */
function activityRouteParameters(): array
{
    $found = [];
 ...
 */
function activityBoundParameters(): array
{
    $names = [];
    $calls = 0;
...
it('mounts the CP routes with SubstituteBindings, so a sibling binding is visible here', function (): void {
    ...
    $this->get(cp_route('activity.show', 1))->assertStatus(418);
});
...
it('binds only parameter names that belong to this addon', function (): void {
    ...
    foreach ($bound['names'] as $parameter) {
        if (! preg_match('/^activity[A-Z][A-Za-z0-9]*$/', $parameter)) {
    ...
    expect($generic)->toBe([], implode("\n", $generic)."\n"
        ...
        .'prefixed `activity` + a capital (activityRecord, …) so no sibling can pick one by '
...
it('keeps the bound names and the route files in agreement', function (): void {
    $bound = activityBoundParameters()['names'];
    $declared = array_keys(activityRouteParameters());
    ...
    expect($unbound)->toBe(
        ...
        .'a binding, rename it to `activity…` in the same commit.'
    );
});
...
it('uses no route parameter a third-party package binds', function (): void {
    ...
    foreach (activityRouteParameters() as $parameter => $files) {
